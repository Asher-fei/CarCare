package com.midian.base.afinal.utils;

import com.midian.base.afinal.annotation.sqlite.Id;
import com.midian.base.afinal.annotation.sqlite.Table;
import com.midian.base.afinal.db.sqlite.ManyToOneLazyLoader;
...
import com.midian.base.afinal.exception.DbException;
...
public class ClassUtils {
	...
	public static String getPrimaryKeyColumn(Class<?> clazz) {
		...
		if(fields != null){
			...
			}else{ //没有ID注解,默认去找 _id 和 userId 为主键，优先寻找 _id
		...
		try {
			...
			for (Field f : fs) {
				if (!FieldUtils.isTransient(f) && FieldUtils.isOneToMany(f)) {
					...
					com.midian.base.afinal.annotation.sqlite.OneToMany oneToMany = f.getAnnotation(com.midian.base.afinal.annotation.sqlite.OneToMany.class);
